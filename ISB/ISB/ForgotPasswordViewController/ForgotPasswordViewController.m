...
#define kmsgSent @"Your password has been sent successfully to your registered email."
...
//    {
  {
    ...
    else if ([[inResponseDic objectAtIndex:0] isEqualToString:@"error"])
    {
        BOOL isSentReceived = NO;
        if ([inResponseDic count] > 1)
            isSentReceived = [Utils checkIfString:[inResponseDic objectAtIndex:1] contains:@"sent"];
        
        if (isSentReceived)
        {
            [Utils showAlertView:kAlertTitle message:kmsgSent delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        }
        else
        {
